...
import { CONTAINER_MAX_OUTPUT_SIZE, CONTAINER_TIMEOUT, DATA_DIR, GROUPS_DIR, IDLE_TIMEOUT, TIMEZONE, } from './config.js';
...
function buildDirectEnv(group, isMain) {
    const projectRoot = process.cwd();
    const groupDir = resolveGroupFolderPath(group.folder);
    const groupIpcDir = resolveGroupIpcPath(group.folder);
    const globalDir = path.join(GROUPS_DIR, 'global');
...
export async function runContainerAgent(group, input, onProcess, onOutput) {
    ...
    const agentRunnerPath = path.join(process.cwd(), 'container', 'agent-runner', 'dist', 'index.js');
    logger.info({ group: group.name, processName: containerName, isMain: input.isMain }, 'Spawning direct agent (no-docker mode)');
    ...
    return new Promise((resolve) => {
        ...
        container.on('close', (code) => {
            ...
            if (isVerbose || isError) {
                ...
                logLines.push(`=== Stderr${stderrTruncated ? ' (TRUNCATED)' : ''} ===`, stderr, ``, `=== Stdout${stdoutTruncated ? ' (TRUNCATED)' : ''} ===`, stdout);
            }
            else {
                logLines.push(`=== Input Summary ===`, `Prompt length: ${input.prompt.length} chars`, `Session ID: ${input.sessionId || 'new'}`, ``);
            }
